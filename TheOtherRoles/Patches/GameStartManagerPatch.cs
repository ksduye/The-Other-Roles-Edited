            ...
            public static void Postfix(GameStartManager __instance) {
                ...
                foreach (InnerNet.ClientData client in AmongUsClient.Instance.allClients.ToArray()) {
                    ...
                    if (dummyComponent != null && dummyComponent.enabled)
                        ...
                        message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} 使用了不同版本的The Other Roles Edited\n</color>";
                    } else {
                        ...
                        if (diff > 0) {
                            ...
                            message += $"<color=#FF0000FF>{client.Character.Data.PlayerName} 使用了其他TOR模组的分支其他分支版本号 v{playerVersions[client.Id].version.ToString()} <size=30%>({PV.guid.ToString()})</size>\n</color>";
                            versionMismatch = true;
                        }
                    }
                }
                ...
                // Client update with handshake infos
                    ...
                    if (!playerVersions.ContainsKey(AmongUsClient.Instance.HostId) || TheOtherRolesEditedPlugin.Version.CompareTo(playerVersions[AmongUsClient.Instance.HostId].version) != 0) {
                        ...
                        __instance.GameStartText.text = $"<color=#FF0000FF>房主使用了不同版本的The Other Roles Edited\n你将在 {Math.Round(10 - kickingTimer)}秒内被踢出</color>";
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition + Vector3.up * 2;
                    } else if (versionMismatch) {
                        __instance.GameStartText.text = $"<color=#FF0000FF>玩家使用了不同版本的The Other Roles Edited:\n</color>" + message;
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition + Vector3.up * 2;
                    } else {
                        __instance.GameStartText.transform.localPosition = __instance.StartButton.transform.localPosition;
                        if (__instance.startState != GameStartManager.StartingStates.Countdown && startingTimer <= 0) {
                            ...
                            __instance.GameStartText.text = $"开始倒计时 {(int)startingTimer + 1}";
                            if (startingTimer <= 0) {
                                __instance.GameStartText.text = String.Empty;
                            }
                        }
