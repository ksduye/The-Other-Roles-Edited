...
public static class EventUtility {
    ...
    public static void Update() {
        if (!isEnabled || eventQueue == null || AmongUsClient.Instance.GameState != InnerNet.InnerNetClient.GameStates.Started || TheOtherRolesEdited.rnd == null || IntroCutscene.Instance) return;
        foreach (EventTypes curEvent in eventQueue.ToArray()) {
            if (TheOtherRolesEdited.rnd.NextSingle() < eventProbabilities[(int)curEvent]) {
    ...
    public static void StartEvent(EventTypes eventToStart) {
        ...
        switch (eventToStart) {
            ...
            case EventTypes.Communication:
                int index = TheOtherRolesEdited.rnd.Next(relevantPlayers.Count);
                ...
                foreach (CachedPlayer pc in relevantPlayers.ToArray()) {
                    if (TheOtherRolesEdited.rnd.NextSingle() < 1f / relevantPlayers.Count) {
                ...
                msg += lastRoom != ""? lastRoom : (TheOtherRolesEdited.rnd.Next(2) > 0 ? "sus" : "safe");
