...
namespace TheOtherRolesEdited.Objects {
    public class Silhouette {
        ...
        public Silhouette(Vector3 p, float duration = 1f, bool visibleForEveryOne = true) {
            if (duration <= 0f) {
                TheOtherRolesEditedPlugin.Logger.LogMessage("silhouette: permanent!");
                permanent = true;
            }
        ...
        public static void UpdateAll() {
            foreach (Silhouette current in new List<Silhouette>(silhouettes)) {
                ...
                if (current.timeRemaining < 0 && !current.permanent) {
                    TheOtherRolesEditedPlugin.Logger.LogMessage($"update: permanent: {current.permanent}, time: {current.timeRemaining}");
